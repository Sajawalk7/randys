...
class WC_Address_Validation_Admin {
	...
	public function set_wc_settings_page_name() {
		$this->wc_settings_page_name = SV_WC_Plugin_Compatibility::normalize_wc_screen_id();
	}
	...
	public function get_settings_page() {
		...
		if ( ! $this->settings_page instanceof WC_Settings_Address_Validation ) {
			...
			$this->settings_page = wc_address_validation()->load_class( '/includes/admin/class-wc-settings-address-validation.php', 'WC_Settings_Address_Validation' );
		}
		...
		if ( $this->wc_settings_page_name === $screen->id ) {
			...
			if ( 'address_validation' === $current_tab ) {
				...
				wp_localize_script( 'wc_address_validation_settings', 'wc_address_validation_settings', array(
					...
					'active_provider' => get_class( wc_address_validation()->get_handler_instance()->get_active_provider() ),
					'i18n' => array(
